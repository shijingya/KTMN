...
class BaseDataSet(Data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frcn_feat_iter, grid_feat_iter, bbox_feat_iter, w_feat_iter, h_feat_iter = self.load_img_feats(idx, iid)

        return \
            torch.from_numpy(frcn_feat_iter),\
            torch.from_numpy(grid_feat_iter),\
            torch.from_numpy(bbox_feat_iter), \
            torch.from_numpy(w_feat_iter), \
            torch.from_numpy(h_feat_iter), \
...
class BaseAdapter(nn.Module):
    ...
    def forward(self, frcn_feat, grid_feat, bbox_feat, w_feat, h_feat):
        feat_dict = feat_filter(self.__C.DATASET, frcn_feat, grid_feat, bbox_feat, w_feat, h_feat)
